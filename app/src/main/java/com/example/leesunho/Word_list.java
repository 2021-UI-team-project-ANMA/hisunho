...
public class Word_list extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        databaseReference.child("단어장").child("내 단어장").child("수능 필수 영단어 100").child("1").addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                //파이어베이스 데이터베이스의 데이터를 받아오는 곳
                arrayList.clear();
                //for(DataSnapshot snapshot : dataSnapshot.getChildren()){ //반복문으로 데이터 List를 추출
                    Word word = dataSnapshot.getValue(Word.class); // 만들어둔 객체에 데이터를 담는다.
                    arrayList.add(word);
                //}
